...
(function() {
    ...
    // SECURITY: never build table markup by concatenating server data into
    // innerHTML. `path`, `referrer`, etc. are attacker-controlled (any
    // anonymous visitor can request an arbitrary path) and are logged even
    // for 404s. Always create cells with textContent so the browser treats
    // the value as plain text, never as markup.
    ...
    function fetchRealtimeData() {
        fetch(dataUrl)
            ...
            .then(function(data) {
                ...
                var apiCount = document.getElementById('apiCount');
                ...
                if (apiCount) apiCount.textContent = data.api_calls || 0;
