...
def _latest(name: str):
    ...
    try:
        req = Request(url, headers={"User-Agent": "Paxalia-Dashboard/3.0.0 dependency-health"})
...
def dependencies(request):
    ...
    return render(
        request,
        "analytics/dependencies.html",
        {
            "active_page": "security_dependencies",
...
def dependency_status(request, package_name: str):
    ...
    return JsonResponse({
        ...
        "latest": latest,
        "source": "PyPI" if latest else None,
        "status": status,
    })
