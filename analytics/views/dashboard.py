...
def analytics_dashboard(request):
    ...
    today_unique = PageView.objects.filter(created_at__date=today).values('ip_hash').distinct().count()
    ...
    try:
        ...
        yest_pageviews = PageView.objects.filter(created_at__date=yesterday)
    ...
    today_pageviews = PageView.objects.filter(created_at__date=today)
    ...
    if compare_active:
        ...
        prev_qs = PageView.objects.filter(created_at__range=(prev_start, prev_end))
        prev_daily = (
            prev_qs
            .annotate(day=TruncDate('created_at'))
            .values('day')
