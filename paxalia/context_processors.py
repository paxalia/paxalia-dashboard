...
def analytics_config(request):
    ...
    upload_init_url = reverse('paxalia:upload_init')
    upload_chunk_url = reverse('paxalia:upload_chunk', kwargs={'upload_id': dummy_id}).replace(str(dummy_id), 'PLACEHOLDER')
    upload_complete_url = reverse('paxalia:upload_complete', kwargs={'upload_id': dummy_id}).replace(str(dummy_id), 'PLACEHOLDER')
    upload_delete_url = reverse('paxalia:upload_delete', kwargs={'upload_id': dummy_id}).replace(str(dummy_id), 'PLACEHOLDER')
    upload_list_url = reverse('paxalia:upload_list')

    return {
        'sidebar_sections': config['SIDEBAR_SECTIONS'],
        'billing_available': 'billing' in config['SIDEBAR_SECTIONS'],
        # ─── API endpoints ──────────────────────────────────────────
        'ANALYTICS_REALTIME_DATA_URL': reverse('paxalia:realtime_data'),
        'ANALYTICS_SERVER_METRICS_URL': reverse('paxalia:api_server_metrics'),
        'ANALYTICS_SERVER_HISTORY_URL': reverse('paxalia:api_server_history'),
        ...
        'ANALYTICS_EVENTS_DATA_URL': reverse('paxalia:events'),
