...
def _resolve_site_id(request):
    ...
    cache_key = f'paxalia:site_for_host:{host}'
...
class AnalyticsMiddleware:
    ...
    def __call__(self, request):
        ...
        # 3b. Determine if this is an API call, so page-view paxalia
        # (Overview/Pages/Traffic/Geography/Realtime) and API paxalia
        ...
        # gate. Mirrors the client-side gate in paxalia-events.js
...
class SecurityBlockMiddleware:
    ...
    Center's blocklist (paxalia.models.BlockedIP).
    ...
    *before* AnalyticsMiddleware, so blocked requests never even get
    ...
    def __call__(self, request):
        ...
        if ip:
            cache_key = f'paxalia:blocked_ip:{ip}'
