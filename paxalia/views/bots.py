from django.shortcuts import render
from ..admin_security import admin_security_required
from django.db.models import Count
from django.utils import timezone
from datetime import timedelta
from ..models import PageView, DailySiteStats
from .utils import get_current_site, site_scoped

@admin_security_required
def bots_overview(request):
    today = timezone.now().date()
    last_30_days = today - timedelta(days=30)
    current_site = get_current_site(request)

    total_bot_views = site_scoped(PageView.objects.filter(is_bot=True), current_site).count()
    bot_views_today = site_scoped(
        PageView.objects.filter(is_bot=True, created_at__date=today), current_site
    ).count()
    unique_bot_ips = site_scoped(
        PageView.objects.filter(is_bot=True), current_site
    ).values('ip_hash').distinct().count()

    # Daily bot views (last 30 days)
    stats = DailySiteStats.objects.filter(date__gte=last_30_days)
    if current_site is not None:
        stats = stats.filter(site=current_site)
    stats = stats.order_by('date')
    dates = [s.date.isoformat() for s in stats]
    bot_counts = [s.bot_views for s in stats]

    # Top bot paths (last 30 days)
    top_paths_qs = (
        site_scoped(
            PageView.objects.filter(is_bot=True, created_at__date__gte=last_30_days),
            current_site,
        )
        .values('path')
        .annotate(count=Count('id'))
        .order_by('-count')[:20]
    )
    top_paths = [item['path'] for item in top_paths_qs]
    top_counts = [item['count'] for item in top_paths_qs]

    # Bot requests by country (last 30 days)
    country_qs = (
        site_scoped(
            PageView.objects.filter(is_bot=True, created_at__date__gte=last_30_days),
            current_site,
        )
        .values('country_code')
        .annotate(count=Count('id'))
        .order_by('-count')[:10]
    )
    country_codes = [c['country_code'] or 'Unknown' for c in country_qs]
    country_counts = [c['count'] for c in country_qs]

    # Bot category breakdown (last 30 days) — Phase 10. Categories are
    # set at write time by bot_classification.py; see that module for
    # what each one means and the BREAKING CHANGE note on is_bot itself.
    category_qs = (
        site_scoped(
            PageView.objects.filter(is_bot=True, created_at__date__gte=last_30_days),
            current_site,
        )
        .exclude(bot_category='')
        .values('bot_category')
        .annotate(count=Count('id'))
        .order_by('-count')
    )
    category_labels = dict(PageView._meta.get_field('bot_category').choices)
    category_breakdown = [
        {'category': row['bot_category'], 'label': category_labels.get(row['bot_category'], row['bot_category']), 'count': row['count']}
        for row in category_qs
    ]

    bot_data = {
        'dates': dates,
        'bot_counts': bot_counts,
        'top_paths': top_paths,
        'top_counts': top_counts,
        'country_codes': country_codes,
        'country_counts': country_counts,
        'category_labels': [c['label'] for c in category_breakdown],
        'category_counts': [c['count'] for c in category_breakdown],
    }

    context = {
        'active_page': 'bots',
        'page_title': 'Bot Traffic',
        'page_subtitle': 'Analysis of scanner and bot requests',
        'total_bot_views': total_bot_views,
        'bot_views_today': bot_views_today,
        'unique_bot_ips': unique_bot_ips,
        'bot_data': bot_data,
        'category_breakdown': category_breakdown,
    }
    return render(request, 'paxalia/bots.html', context)