// paxalia/static/paxalia/scripts/server/cpu.js
...
(function() {
    ...
    if (!API_URL || !HISTORY_URL) {
        console.warn('[Paxalia] Server API URLs not defined. CPU charts will not work.');
        return;
    }
    ...
    function updateCPU() {
        fetch(API_URL)
            ...
            .then(data => {
                const cpu = data.cpu || {};
                document.getElementById('cpu-overall').textContent = (cpu.percent || 0) + '%';
                document.getElementById('cpu-cores').textContent = cpu.count || 0;
                if (cpu.load_avg) {
                    document.getElementById('load-avg').textContent = cpu.load_avg.join(' / ');
                }
